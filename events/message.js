...
//Mention and respond
  ...
  if ([`<@${client.user.id}>`, `<@!${client.user.id}>`].includes(message.content) && !args[0]) {
    let embed = new MessageEmbed()
      ...
      .addField("Support Server:", `**[Click Here!](https://discord.gg/hPHvF8BggE)**`)
      .setColor(color)
...
//Blacklist and Suspend
  ...
  if (message.content.startsWith(guild.prefix.toLowerCase()) && guild.blacklist == true) return message.channel.send(">  **This server has been blacklisted. Join support server to appeal. https://discord.gg/hPHvF8BggE**");



  if (user && user.blacklist == true && message.content.toLowerCase().startsWith(guild.prefix)) return message.channel.send(">  **You have been blacklisted. Join support server to appeal. https://discord.gg/hPHvF8BggE**");
  ...
  await get(options).then(async t => {
    ...
    context.drawImage(pk, 140, 50, 350, 350);
        const embed = new MessageEmbed()
            .setAuthor(`New Pokemon Came To You`)
