...
module.exports = {
    ...
    execute: async (client, message, args, prefix, guild, color, channel) => {
        ...
        if (!spawn.pokemon[0]) return message.channel.send(`> ${config.no} **There is no pokemon here!**`);

        if (spawn.hcool) return message.channel.send(`> ${config.no} **You can't use hint this much quickly, please try after few seconds.**`);
