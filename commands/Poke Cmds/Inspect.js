...
module.exports = {
    ...
    execute: async (client, message, args, prefix, guild, color, channel) => {
        ...
        // if (!user.pokemons[selected]) return message.reply("You Don't have any Pokémon Selected!")
        ...
        if (args[0] && !isNaN(args[0])) {
            selected = parseInt(args[0]) - 1;
            if (!user.pokemons[selected]) return message.channel.send("You don't have a pokémon with this number!");
        } else if (args[0] && (args[0].toLowerCase() === "latest" || args[0].toLowerCase() === "l")) {
        ...
        const gen8 = Gen8.find(e => e.name.toLowerCase() === user.pokemons[selected].name.toLowerCase());
        const form = Forms.find(e => e.name.toLowerCase() === user.pokemons[selected].name.toLowerCase());
        const concept = Concept.find(e => e.name.toLowerCase() === user.pokemons[selected].name.toLowerCase());
        const galarian = Galarians.find(e => e.name.toLowerCase() === user.pokemons[selected].name.toLowerCase().replace("galarian-", ""));
        const forms = Forms.find(e => e.name === name.toLowerCase());
        const mega = Mega.find(e => e.name.toLowerCase() === name.replace("mega-", "").toLowerCase());
        const shadow = Shadow.find(e => e.name.toLowerCase() === name.replace("shadow-", "").toLowerCase());
        const primal = Primal.find(e => e.name === name.replace("primal-", "").toLowerCase());
        const pokemon = Pokemon.find(e => e.name === user.pokemons[selected].name.toLowerCase());
        ...
        level !== 100
            ...
            + `${user.pokemons[selected].xp}/${(1.2 * level ^ 3) - (15 * level ^ 2) + (100 * level) - 140}XP\n`
            : desc = `${(user.pokemons[selected].nick != null ? `**Nickname:** ${user.pokemons[selected].nick}\n` : "")}`
            + `**Max Level**\n`
        ...
        if (gen8) {
            ...
            let hpTotal = Math.floor(Math.floor((2 * hpBase + hp + (0 / 4) * level) / 100) + level + 10),
                ...
                ivTotal = (((hp + atk + def + spatk + spdef + speed) / 186) * 100).toFixed(2);
        ...
        if (galarian && user.pokemons[selected].name.toLowerCase().startsWith("galarian")) {
        ...
        if (mega && name.toLowerCase().startsWith("mega-")) {
            ...
            var url = mega.url;
            shiny = ShinyMega.find(e => e.name === name.replace("mega-", "").toLowerCase())
            if (user.pokemons[selected].shiny && shiny) url = shiny.url;
            if (url !== user.pokemons[selected].url) {
                user.pokemons[selected].url = url
                await user.markModified(`pokemons`);
        ...
        if (pokemon) {
            ...
            } else if (user.pokemons[selected].shiny && user.pokemons[selected].name.toLowerCase() == "meltan") {
        ...
        let t = await get({
            ...
            message.reply("An error occured to info the pokemon. Please contact the developers of the bot")
        })
        ...
        if (name.toLowerCase() == "shellos") url = "https://imgur.com/0TNMAHQ.png"
        if (name.toLowerCase() == "jellicent") url = "https://imgur.com/tMspIRX.png"
        if (user.pokemons[selected].name.toLowerCase() === "giratina-origin" && !user.pokemons[selected].shiny) url = "https://imgur.com/UHVxS2q.png"
        if (user.pokemons[selected].name.toLowerCase() === "eternatus" && user.pokemons[selected].shiny) url = "https://i.imgur.com/lkx7zZ3.png"
        if (user.pokemons[selected].shiny) {
            url = `https://assets.poketwo.net/shiny/${t.id}.png?v=26`
            const pkmn = Shiny.find(e => e.name.toLowerCase().replace("-", " ") === name.toLowerCase().replace("-", " "))

            if (pkmn) url = pkmn.url;
            if (name.toLowerCase() == "giratina-origin") url = `https://imgur.com/UHVxS2q.png`;
            if (name.toLowerCase() == "giratina-altered") url = `https://i.imgur.com/oSe49i5.png`;
            if (name.toLowerCase() === "primal-kyogre") url = "https://i.imgur.com/XdZwD0s.png";
            if (name.toLowerCase() === "primal-groudon") url = "https://i.imgur.com/Xzm1FDn.png";
        }
        if (url !== user.pokemons[selected].url) {
            user.pokemons[selected].url = url
            await user.markModified(`pokemons`);
